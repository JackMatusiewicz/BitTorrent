...
ConnectedSocketClient::ConnectedSocketClient(SocketInfo&& socketInfo)
    : _socketInfo{std::move(socketInfo)}
    , _has_socket(false)
    , _socket_fd(0) {}

Result<int, std::string> ConnectedSocketClient::open_connection() noexcept {
    if (_has_socket) {
...
Result<int, std::string> ConnectedSocketClient::close_connection() noexcept {
    if (!_has_socket) {
...
Result<int, std::string> ConnectedSocketClient::send_data(const std::vector<char>& data) const noexcept {
    if (!_has_socket) {
...
Result<std::vector<char>, std::string> ConnectedSocketClient::receive_data() const noexcept {
    if (!_has_socket) {
